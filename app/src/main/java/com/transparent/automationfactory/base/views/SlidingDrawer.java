...
//		}
		...
		int triggerId = a.getResourceId(R.styleable.SlidingDrawer_trigger, 0);
		if (triggerId == 0) {
			throw new IllegalArgumentException(
					"The handle attribute is required and must refer "
							+ "to a valid child.");
	...
	public boolean onInterceptTouchEvent(MotionEvent event) {
		...
		final Rect trigger = new Rect();
		mTrigger.getHitRect(trigger);
		
		double hitx = x - frame.left;
		double hity = y - frame.top;
		/*
		if(isOpened()) {
			final Rect content = new Rect();
			mContent.getHitRect(content);
			if(content.contains((int)x, (int)y)) {
				return true;
			}
		}
		*/
		final boolean hit = trigger.contains((int) hitx, (int) hity);
		if (!mTracking && !hit) {
			return false;
		}
...
//		return mTracking || mAnimating || super.onTouchEvent(event);
		return isOpened() || mTracking || mAnimating || super.onTouchEvent(event);
	}

	@Override
	protected void onFinishInflate() {
		mHandle = findViewById(mHandleId);
		mTrigger = findViewById(mTriggerId);
		if (mHandle == null) {
			throw new IllegalArgumentException(
		...
		if (mAnimateOnClick) {
			mTrigger.setOnClickListener(new DrawerToggler());
